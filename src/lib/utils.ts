...
// bucket에 들어있는 img 파일의 url 반환
export const getUrl = async (key: string | null, bucket = 'app') => {
	if (!key) {
		return '/empty.png';
	}
...
// bucket에 들어있는 img 파일의 signedUrl 반환
...
// 이미지 리사이징
export const resize = async (file: Blob, width = 400, quality = 0.4) => {
	try {
